...
export async function getAnalyticsVisitors(from, to, limit = 50) {
  const { data, error } = await requireAnalyticsClient().rpc('get_web_analytics_visitors', {
    p_from: from.toISOString(),
    p_to: to.toISOString(),
    p_limit: limit,
  })
...
export async function getAnalyticsVisitorJourney(visitorId, from, to) {
  const { data, error } = await requireAnalyticsClient().rpc('get_web_analytics_visitor_journey', {
