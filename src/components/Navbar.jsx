...
export default function Navbar() {
  ...
  return (
    <header className={`site-navbar authority-navbar ${transparent ? 'site-navbar--hero' : 'site-navbar--solid'} ${hidden ? 'site-navbar--hidden' : ''}`}>
      ...
      <AnimatePresence>
        {mobileOpen ? (
          <motion.nav id="mobile-navigation" className="authority-navbar__mobile" aria-label="Navigation mobile" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}>
            <div className="container-max">
              {nav.map((item, index) => <NavLink key={item.href} to={item.href}><span>{String(index + 1).padStart(2, '0')}</span>{item.label}</NavLink>)}
              <Link to={ctaHref} className="btn-accent">{ctaLabel}</Link>
              {!localized ? <Link to="/login/" className="authority-navbar__login">Espace cabinet</Link> : null}
            </div>
          </motion.nav>
        ) : null}
      </AnimatePresence>
